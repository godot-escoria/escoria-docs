...
available methods, please refer to :doc:`ESCGame </api/supporting_classes/ESCGame>`.
...
.. |use_action| image:: /advanced/img/action_use_button.png
    :alt: Use action verb texture
    :scale: 100%

.. |talk_action| image:: /advanced/img/action_talk_button.png
...
``TextureButton``. You can rename these 2 nodes with each action's name for
clarity.

.. image:: /advanced/img/verbs_scene_tree.png
    :align: center
    :alt: Verbs scene tree

Set each of these buttons' ``texture_normal`` setting to use the appropriate texture.

.. image:: /advanced/img/verbs_scene.png
    :align: center
    :alt: Verbs scene

Now, we need to script this scene so that Escoria's **Action Manager** is
informed that an action button was clicked and therefore needs to change the
current action verb.

Create and attach a new script for the root node that uses the following:
