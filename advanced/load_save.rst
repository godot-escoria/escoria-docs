...
- ``voice_lang``: The audio speech language.
...
- ``sfx_volume``: The sound effects volume, i.e. any sounds that are not music
...
- ``custom_settings``: A dictionary containing any custom data to be saved. For
  example, the screen resolution could be added here. See `Saving
...
The :doc:`ESCSaveSettings </api/supporting_classes/ESCSaveSettings>` data structure contains a
...
With newer versions of Escoria, certain built-in ASHES variables may change their
name. For this reason, upgrading the Escoria version may break older versions
of a save game. In the same way, during the development of the game, an ASHES
variable or item ID may change. It is then necessary to migrate old save games
...
any version requiring a migration must have an associated migration script.
...
Any migration script must extend the :doc:`ESCMigration </api/supporting_classes/ESCMigration>`
class and implement the ``migrate()`` function. This function modifies the save
game content by moving the older variables to the newer ones. To do so, the
...
Save games hold "header" data, including the following:
...
- **Name that describes the save game**: This can be either provided by the player
  or managed by the game (and thus transparent to the player).

- **Date**: the save game's creation date.
